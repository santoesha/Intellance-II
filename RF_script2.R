...
#Samples not seen in train set after fold 1 and fold 2
samples <- read.delim("test_set_samples.txt",header=F)
write(intersect(samples$V1,rownames(test_set)),"samples_notin_train_afterfold2.txt")

# ---- 5 times repeated 10FCV ----
for (i in 1:5) {
  ...
  for(i in 1:10) { 
    ...
    #Write results
    write(t(c(R2_train,R2_val)), 'performance_fold2.txt', append=TRUE)
    write.table(genes, 'boruta_fold2.txt', append=TRUE)
    write.table(var_imp$importance, 'varimp_fold2.txt', append=TRUE)
    write(nrow(genes),"number_of_genes_fold2.txt",append=TRUE)
  }
}

# ---- Combine results  ----
#Performance
performance <- read.delim("performance_fold2.txt",header = FALSE, sep=" ")
...
#Boruta
boruta <- read.delim("boruta_fold2.txt") %>% separate(col = genes, into = c("number", "gene"), sep = " ") %>% drop_na() %>% dplyr::select(-c(1)) %>% dplyr::group_by(gene) %>% summarise(count = n()) 
number_of_genes <- read.delim("number_of_genes_fold2.txt",header=FALSE)
...
#Varimp
varimp <- read.delim("varimp_fold2.txt") %>% separate(col = Overall, into = c("gene", "score"), sep = " ") %>% drop_na() 
